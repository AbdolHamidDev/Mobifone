...
@section('content')
@include('partials.content-header', ['name' => 'Danh sách', 'key' => 'CV'])
...
<div class="container mx-auto p-6">
    ...
    <div class="mb-6 flex items-center space-x-4">
        ...
        id="search-input" 
        name="nganh_nghe" 
        class="w-full p-4 pr-12 border border-gray-300 rounded-lg shadow focus:outline-none focus:ring-2 focus:ring-blue-500">
        <option value="">Chọn ngành nghề hoặc tên ứng viên...</option>
        @foreach($nganhNgheList as $nganhNghe)
            <option value="{{ $nganhNghe }}">{{ $nganhNghe }}</option>
        @endforeach
    </select>
    ...
    <div class="flex flex-wrap -mx-4 min-h-[200px]">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.toggle-view').forEach(link => {
            link.addEventListener('click', function (e) {
                ...
                link.textContent = shortText.classList.contains('hidden') ? 'Thu gọn' : 'Xem thêm';
            });
        });
