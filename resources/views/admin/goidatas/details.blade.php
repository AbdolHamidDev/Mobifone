                ...
                @foreach ($details as $detail)
                    @if ($detail->goidata_id != $previousgoidataId)
                        @php
                            ...
                            $goidata = $detail->goidata; // Lấy tên gói cước từ quan hệ
                        @endphp
    
                        <div class="bg-white shadow-lg rounded-lg">
                            <button class="w-full text-left p-4 flex items-center justify-between focus:outline-none accordion-header" data-goidata_id="{{ $detail->goidata_id }}">
                                <span class="text-lg font-semibold text-gray-800">ID: {{ $detail->goidata_id }} - {{ $goidata->ten_data }}</span>
                                        ...
                                        @foreach ($details->where('goidata_id', $previousgoidataId) as $item)
                                            <tr>
                                                ...
                                                <td class="border px-4 py-2 flex space-x-2">
                                                    <a href="{{ route('goidatas_detail.edit', $item->id) }}" class="text-yellow-500 hover:text-yellow-700">
